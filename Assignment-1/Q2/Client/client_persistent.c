...
{
  struct sockaddr_in address; //this is the address of the client
  int sock = 0, valread;
  struct sockaddr_in serv_addr; //this is the address of the server
  char *hello = "Hello from client";
  char *done = "Done"; // these messages are to make sure that the connection as been established
  int i, stillRead=1;
  ...
  while(1){  
    ...
    fflush(stdout); //empty the buffer in order to remove any remaining or extra bytes, terminating charaters and so on
    scanf("%s", filename); 
